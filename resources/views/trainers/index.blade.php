...
@section('title', __('Trainers Directory'))
@section('meta_description', __('Manage gym trainers and staff.'))
@section('page_title', __('Trainers Directory'))
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            <div class="px-4 py-3 bg-white rounded-2xl shadow-sm border border-gray-100 flex items-center gap-3">
                ...
                <div>
                    <p class="text-xs font-medium" style="color: #6B7280;">{{ __('Total Trainers') }}</p>
            ...
            <div class="px-4 py-3 bg-white rounded-2xl shadow-sm border border-gray-100 flex items-center gap-3">
                ...
                <div>
                    <p class="text-xs font-medium" style="color: #6B7280;">{{ __('Active') }}</p>
            ...
            <div class="px-4 py-3 bg-white rounded-2xl shadow-sm border border-gray-100 flex items-center gap-3">
                ...
                <div>
                    <p class="text-xs font-medium" style="color: #6B7280;">{{ __('Inactive') }}</p>
        ...
        <a href="{{ route('trainers.create') }}"
           ...
           style="background-color: #22C55E;">
            <i data-lucide="plus" class="w-4 h-4"></i> {{ __('Add Trainer') }}
        </a>
    </div>
    ...
    <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        <form method="GET" action="{{ route('trainers.index') }}" class="flex flex-col md:flex-row gap-4" id="filterForm">
            <div class="flex-1 relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Search name, email, phone, or specialization...') }}"
            ...
            <div class="w-full md:w-48">
                <select name="status" onchange="document.getElementById('filterForm').submit()"
                    ...
                    <option value="all" {{ request('status') === 'all' || !request('status') ? 'selected' : '' }}>{{ __('All Statuses') }}</option>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        @if($trainers->isEmpty())
            <div class="flex flex-col items-center justify-center py-16 text-center px-6">
                ...
                <p class="text-base font-semibold mb-1" style="color:#111827;">{{ __('No trainers found') }}</p>
                <p class="text-sm mb-4" style="color:#6B7280;">
                    @if(request('search') || (request('status') && request('status') !== 'all'))
                        {{ __('No trainers match your search criteria.') }}
                        <a href="{{ route('trainers.index') }}" class="underline" style="color:#22C55E;">{{ __('Clear filters') }}</a>
                    @else
                        {{ __('Start by adding your first trainer.') }}
                    @endif
                </p>
                @if(!request('search') && !request('status'))
                    <a href="{{ route('trainers.create') }}"
                       ...
                       style="background-color:#22C55E;">
                        <i data-lucide="plus" class="w-4 h-4"></i> {{ __('Add First Trainer') }}
                    </a>
                @endif
            </div>
        @else
        <table class="w-full text-sm text-left">
            <thead class="bg-gray-50 text-xs uppercase tracking-wider" style="color: #9CA3AF;">
                <tr>
                    ...
                    <th class="px-6 py-4 font-medium hidden md:table-cell">{{ __('Specialization') }}</th>
                    <th class="px-6 py-4 font-medium hidden lg:table-cell">{{ __('Joined') }}</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 hidden md:table-cell" style="color: #374151;">
                        {{ $trainer->specialization ?? __('General') }}
                    </td>
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('trainers.show', $trainer) }}"
                               ...
                               style="color:#6B7280;" title="{{ __('View Profile') }}">
                            ...
                            <a href="{{ route('trainers.edit', $trainer) }}"
                               ...
                               style="color:#6B7280;" title="{{ __('Edit') }}">
                            ...
                            <form action="{{ route('trainers.toggle-status', $trainer) }}" method="POST" class="inline">
                                ...
                                <button type="submit"
                                        ...
                                        title="{{ $trainer->is_active ? __('Deactivate') : __('Activate') }}">
