...
@section('page_title', __('Expense Report'))
...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form method="GET" action="{{ route('reports.expenses') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div class="md:col-span-4 flex justify-end gap-2 mt-2">
                <a href="{{ route('reports.expenses') }}" class="px-6 py-2 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-colors text-sm font-medium">{{ __('Reset') }}</a>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex items-center gap-4">
            ...
            <div>
                <p class="text-sm font-medium text-gray-500">{{ __('Expense Count') }}</p>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 lg:row-span-1 lg:col-span-1">
            <h4 class="text-sm font-bold text-gray-900 mb-2">{{ __('Category Totals') }}</h4>
            <div class="space-y-2 max-h-24 overflow-y-auto">
                ...
                @empty
                    <div class="text-xs text-gray-500">{{ __('No categories found for this period.') }}</div>
                @endforelse
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h3 class="text-lg font-bold text-gray-900">{{ __('Expense Records') }}</h3>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($expensesList as $expense)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">{{ $expense->expense_date->gymDateFormat() }}</td>
