...
@section('meta_description', 'Full payment history for all members.')
...
<form method="GET" action="{{ route('fees.history') }}" class="flex flex-col sm:flex-row gap-3 mb-6">
    ...
    <input type="month" name="month" value="{{ request('month') }}" onchange="this.form.submit()"
           class="px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none">
    <button type="submit" class="px-4 py-2 rounded-xl text-white text-sm font-medium" style="background-color:#22C55E;">Filter</button>
    @if(request()->hasAny(['search', 'method', 'month']))
        <a href="{{ route('fees.history') }}" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-sm font-medium hover:bg-gray-200">Clear</a>
    @endif
</form>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left text-sm whitespace-nowrap">
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="font-medium text-gray-900">No payments found</p>
                            <p class="text-gray-500 text-sm mt-1">Adjust your filters to see results.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
            ...
            <tfoot class="border-t-2 border-gray-200 bg-gray-50">
                <tr>
                    <td colspan="4" class="px-6 py-3 text-sm font-semibold text-gray-700">Page Total</td>
