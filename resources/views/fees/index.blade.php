...
@section('meta_description', 'Manage member fees, record payments, and track outstanding balances.')
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4">
        ...
        <div>
            <p class="text-sm text-gray-500 font-medium">Pending / Outstanding</p>
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4">
        ...
        <div>
            <p class="text-sm text-gray-500 font-medium">This Month's Revenue</p>
...
<div class="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6">
    <div class="flex items-center gap-2 mb-3">
        ...
        <h3 class="text-sm font-semibold text-amber-800">Memberships Expiring Within 7 Days</h3>
    </div>
    <div class="space-y-2">
        ...
        <div class="flex items-center justify-between bg-white rounded-xl px-4 py-3 border border-amber-100">
            <div>
                ...
                <span class="text-sm font-medium {{ $reminder->end_date->isPast() ? 'text-red-600' : 'text-amber-700' }}">
                    Expires {{ $reminder->end_date->format('d M Y') }}
                    @if($reminder->end_date->isPast()) <span class="text-xs">(Overdue)</span>
                    @elseif($reminder->end_date->isToday()) <span class="text-xs">(Today)</span>
...
<div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden mb-6">
    <div class="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <form method="GET" action="{{ route('fees.index') }}" class="flex gap-2">
            ...
            @if(request('search'))
                <a href="{{ route('fees.index') }}" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-sm font-medium hover:bg-gray-200">Clear</a>
            @endif
        </form>
    </div>
        ...
        <table class="w-full text-left text-sm whitespace-nowrap">
            <thead class="bg-gray-50 border-b border-gray-200 text-gray-500">
                <tr>
                    ...
                    <th class="px-6 py-4 font-medium text-right">Paid</th>
                    <th class="px-6 py-4 font-medium text-right">Remaining</th>
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <span class="text-sm {{ $isOverdue ? 'text-red-600 font-semibold' : ($isDueSoon ? 'text-amber-600 font-medium' : 'text-gray-600') }}">
                            {{ $mm->end_date->format('d M Y') }}
                            @if($isOverdue) <span class="block text-xs text-red-500">Overdue</span>
                            @elseif($isDueSoon) <span class="block text-xs text-amber-500">Due soon</span>
                            @endif
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        @if($mm->status === 'expiring_soon')
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-700">Expiring Soon</span>
                        @else
                ...
                <tr>
                    <td colspan="8" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="font-medium text-gray-900">No pending fees!</p>
                            <p class="text-gray-500 text-sm mt-1">All active memberships are fully paid.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
...
<div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
    <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
        ...
        <a href="{{ route('fees.history') }}" class="text-sm font-medium" style="color:#22C55E;">View All</a>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full text-left text-sm whitespace-nowrap">
            <thead class="bg-gray-50 border-b border-gray-200 text-gray-500">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-6 py-10 text-center text-sm text-gray-500">No payments recorded yet.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
