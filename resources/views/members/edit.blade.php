                ...
                <div class="md:col-span-2">
                    ...
                    @if($member->profile_photo)
                        <div class="mb-3 flex items-center gap-4">
                            ...
                            <span class="text-sm text-gray-500">{{ __('Current photo') }}</span>
                        </div>
                    @endif
                    ...
                    <p class="text-xs mt-1" style="color:#6B7280;">{{ __('Upload a new photo to replace the current one. Max size: 2MB. Formats: JPEG, PNG, WEBP.') }}</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        ...
                        <option value="suspended"     {{ old('status', $member->status) === 'suspended'     ? 'selected' : '' }}>{{ __('Suspended') }}</option>
                        <option value="expired"       {{ old('status', $member->status) === 'expired'       ? 'selected' : '' }}>{{ __('Expired') }}</option>
                        <option value="expiring_soon" {{ old('status', $member->status) === 'expiring_soon' ? 'selected' : '' }}>{{ __('Expiring Soon') }}</option>
                    </select>
                </div>
            </div>
        </div>
