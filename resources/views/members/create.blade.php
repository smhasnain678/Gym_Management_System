...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // ── Offline-First: Add Member form ───────────────────────────────────
        // When ONLINE  → native form submission proceeds unchanged.
        // When OFFLINE → intercept, queue as `member_create` via the existing
        //                IndexedDB action_queue, then give feedback and redirect.
        const form = document.getElementById('create-member-form');
        if (!form) return;

        form.addEventListener('submit', async function (e) {
            // If we are online let the browser submit normally — do nothing.
            if (navigator.onLine) return;

            // ── We are OFFLINE ────────────────────────────────────────────────
            e.preventDefault();

            // Collect all named form values except the file input.
            ...
            // Detect if the user had selected a photo while offline
            const photoInput = document.getElementById('profile_photo');
            const hadPhoto = photoInput && photoInput.files && photoInput.files.length > 0;

            try {
                // Queue via the existing offline helper (same as trainer_create pattern)
                await window.WarmUpOffline.queueMemberCreate(payload);
            } catch (err) {
                console.error('[WarmUp Offline] Failed to queue member_create:', err);
                // Surface a brief inline error and abort so user knows something went wrong
                showOfflineBanner(
                    'Could not save member offline. Please try again.',
                    '#FEE2E2', '#DC2626'
                );
                return;
            }
            ...
            let msg = 'Member saved offline and will sync automatically when you reconnect.';
            if (hadPhoto) {
                msg += ' Profile photo could not be saved offline — please add it when back online.';
            }

            // Show a brief inline banner that mirrors the app's flash-message style,
            // then redirect to the members list (same destination as the online success redirect).
            showOfflineBanner(msg, '#DCFCE7', '#15803D');

            // Short delay so the user can read the message, then navigate away
            setTimeout(function () {
