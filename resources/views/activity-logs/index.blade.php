...
@section('meta_description', __('View all system activity logs.'))
@section('page_title', __('Activity Logs'))
    ...
    <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        <form method="GET" action="{{ route('activity-logs.index') }}" class="flex flex-col md:flex-row gap-4" id="filterForm">
            <div class="flex-1 relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Search description or action...') }}"
            ...
            <div class="w-full md:w-48">
                <select name="action_filter" onchange="document.getElementById('filterForm').submit()"
                    ...
                    <option value="all" {{ request('action_filter') === 'all' || !request('action_filter') ? 'selected' : '' }}>{{ __('All Actions') }}</option>
            ...
            <div class="w-full md:w-48">
                <input type="date" name="date_from" value="{{ request('date_from') }}" onchange="document.getElementById('filterForm').submit()"
                       class="w-full px-4 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:outline-none" title="{{ __('Date From') }}">
            </div>
            
            <div class="w-full md:w-48">
                <input type="date" name="date_to" value="{{ request('date_to') }}" onchange="document.getElementById('filterForm').submit()"
                       class="w-full px-4 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:outline-none" title="{{ __('Date To') }}">
            </div>
            ...
            @if(request('search') || (request('action_filter') && request('action_filter') !== 'all') || request('date_from') || request('date_to'))
                <a href="{{ route('activity-logs.index') }}"
                   ...
                   style="color:#6B7280;">
                    <i data-lucide="x" class="w-4 h-4 mr-1"></i> {{ __('Clear') }}
                </a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        @if($logs->isEmpty())
            <div class="flex flex-col items-center justify-center py-16 text-center px-6">
                ...
                <h3 class="text-lg font-bold text-gray-900 mb-1">{{ __('No activity found') }}</h3>
                <p class="text-gray-500 text-sm max-w-sm">{{ __('No logs match your current filters.') }}</p>
            </div>
        @else
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                            <th scope="col" class="px-6 py-4 font-semibold text-gray-600">{{ __('Timestamp') }}</th>
                            <th scope="col" class="px-6 py-4 font-semibold text-gray-600">{{ __('User') }}</th>
                            <th scope="col" class="px-6 py-4 font-semibold text-gray-600">{{ __('Action') }}</th>
                            <th scope="col" class="px-6 py-4 font-semibold text-gray-600">{{ __('Description') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        @foreach($logs as $log)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    <span class="text-gray-900">{{ $log->created_at->gymDateFormat() }}</span><br>
                                    <span class="text-gray-500 text-xs">{{ $log->created_at->gymTimeFormat() }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-gray-900 font-medium">{{ $log->user->name ?? __('System') }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
